...
static void setting_observer_list_cb(lv_observer_t *observer, lv_subject_t *subject);

lv_obj_t* working_page_setting_init(lv_obj_t *page)
{
    working_page_setting_t *p = lv_malloc(sizeof (working_page_setting_t));
    ...
    static lv_point_precise_t line_points1[] ={{379, 398 - 60}, {902, 398 - 60}};
    ...
    static lv_point_precise_t line_points2[] ={{379, 474 - 60}, {902, 474 - 60}};
    temp_state = lv_line_create(obj);
    lv_obj_add_style(temp_state,theme_style_get_line_green(), 0);
    lv_line_set_points(temp_state, line_points2, 2);

    static lv_point_precise_t line_point3[] ={{379, 550 - 60}, {902, 550 - 60}};
    temp_state = lv_line_create(obj);
    lv_obj_add_style(temp_state,theme_style_get_line_green(), 0);
...
static void lights_observer_list_cb(lv_observer_t *observer, lv_subject_t *subject)
{
